...
# --- 4. HTML PARCALARI (YENI TASARIM) ---

# Parca 1: CSS (Glassmorphism ve Animasyonlar)
h1 = """<!DOCTYPE html><html><head><title>PicoSpace V21</title><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
# Parca 2: HTML Govde (Yeni Menu Yapisi)
...
        <div style="margin-top:20px;color:#aaa;font-size:14px">V21.0 - ULTRA</div>
    </div>
</div>
...
if(!window.THREE) err();
var act=false;
let sc=0,lv=1,sp=0.3,lst=0,tx=0,ty=0,cx=0,cy=0;
...
    if(t-lst>Math.max(250,1000-lv*120)){
...
        lst=t;
    }
    for(let i=bons.length-1;i>=0;i--){let b=bons[i]; b.position.z+=sp+0.3; b.rotation.y+=0.1; if(b.position.z>-1 && b.position.z<1 && shp.position.distanceTo(b.position)<1.6){scn.remove(b);bons.splice(i,1);sc+=50;document.getElementById('sc').innerText=sc; showPop(); if(sc%150<50 && sc>150){lv=Math.floor(sc/150)+1;sp=0.3+(lv*0.05);document.getElementById('lv').innerText=lv;up(lv-1);}} else if(b.position.z>8){scn.remove(b);bons.splice(i,1);}}
    for(let i=ens.length-1;i>=0;i--){let e=ens[i];e.position.z+=sp+0.3;e.rotation.x+=0.04; if(e.position.z>-1&&e.position.z<1&&shp.position.distanceTo(e.position)<1.6){act=false;document.getElementById('es').innerText=sc;document.getElementById('ui').style.display='none';document.getElementById('go').style.display='flex';} if(e.position.z>8){scn.remove(e);ens.splice(i,1);sc+=10;document.getElementById('sc').innerText=sc;if(sc%150==0){lv++;sp+=0.05;document.getElementById('lv').innerText=lv;up(lv-1);}}}
